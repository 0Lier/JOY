...
Cuando decidas ejecutar un comando, EJECÚTALO. No anuncies que lo vas a correr, no pidas permiso, no escribas el comando en texto plano ni en bloques de código markdown (``` ```), y no digas frases como "vamos a ver qué nos muestra" o "podemos ejecutar tal comando". El comando NUNCA debe aparecer visible en tu respuesta hablada — solo debe existir dentro del tag [execute=...]. Tu respuesta hablada antes del tag debe ser corta (una frase, con personalidad) y no debe revelar qué comando es.
- NO USES MARCADOS DE CHAT, TU TEXTO SE MUESTA EN UNA TERMINAL NO EN UNA PAGINA, POR ENDE COSAS COMO *HOLA* SE MUESTRAN IGUAL O ´TAMBIEN´
...
def ejecutar(comando=""):
    ...
    resultado = subprocess.run(
        ...
        stderr=subprocess.STDOUT,  # Fusiona los errores dentro de la salida estándar
        text=True,
        errors="replace",
        shell=True
    )
...
def probar_stream_ollama(prompt_usuario, historial, callback_anim=None):
    ...
    # MATEMÁTICA CORREGIDA: Calculamos el ancho de la columna prompt (2/3 del total) menos el padding del Panel
        ...
        try:
            ...
            for line in response.iter_lines():
                if line:
                    ...
                    # 1. Acumulamos absolutamente todo el texto crudo aquí
                    texto_limpio += token
                    
                    # 2. Sanitizamos el texto para la pantalla usando Regex.
                    # Esto remueve tags completos E incompleteos que se estén streameando.
                    texto_para_pantalla = re.sub(r'\[[^\]]*(?:\]|$)', '', texto_limpio)
                    
                    # 3. Formateamos y envolvemos solo la versión limpia para la consola
                    ...
                    # --- Tu lógica original de backend (animaciones, memoria y ejecución) se mantiene intacta ---
                    texto_acumulado += token.lower()
                    
                    tags_animacion = ["laugh", "sad", "angry", "boring"]
                    for anim in tags_animacion:
                        tag_completo = f"[{anim}]"
                        if tag_completo in texto_acumulado:
                            if callback_anim:
                                callback_anim(anim) 
                            texto_acumulado = texto_acumulado.replace(tag_completo, "")
                            
                    patron = r"\[memory_save=([^%\]]+)%([^\]]+)\]"
                    if match := re.findall(patron, texto_acumulado):
                        for llave, valor in match:
                            save_memory(llave, valor)
                        texto_acumulado = re.sub(patron, "", texto_acumulado)
                    
                    if data.get("done", False):
                        
                        # Limpiamos los tags definitivamente del texto original antes de guardarlo en el historial
                        texto_limpio = re.sub(r'\[[^\]]*\]', '', texto_limpio)
                        mensajes.append({"role": "assistant", "content": texto_limpio})
                        patron = r"\[execute=([^\]]+)\]"
                        if match := re.search(patron, texto_acumulado):
